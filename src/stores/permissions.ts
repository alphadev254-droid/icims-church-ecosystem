import {
  ...
  BookOpen, Building2, TrendingUp, BarChart3, Settings, Shield, UserCog, Package2, type LucideIcon,
} from 'lucide-react';
...
const PERMISSION_TO_NAV: Array<{ permission: string; item: NavItem }> = [
  ...
  { permission: 'churches:read',      item: { to: '/dashboard/churches',      label: 'Churches',      icon: Building2 } },
...
export function getNavForPermissions(permissions: string[]): NavItem[] {
  ...
  return PERMISSION_TO_NAV
    .filter(({ permission }) => permSet.has(permission))
    .map(({ item }) => item);
}
