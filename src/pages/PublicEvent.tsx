...
import { Calendar, MapPin, Clock, Users, Share2, Copy, Check } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container max-w-4xl py-8 space-y-6">
        ...
        {/* Register Button */}
        {event.requiresTicket && event.ticketPrice > 0 && (
          <Card className="bg-accent/10 border-accent">
            <CardContent className="p-6 text-center">
              <p className="text-lg font-semibold mb-2">Ticket Price: {event.currency} {event.ticketPrice}</p>
              <p className="text-sm text-muted-foreground mb-4">
                {event.maxAttendees && `${event.maxAttendees - (event.ticketsSold || 0)} tickets remaining`}
              </p>
              <Button 
                size="lg" 
                className="w-full sm:w-auto"
                onClick={() => window.location.href = `/login?redirect=/dashboard/events/${event.id}/tickets`}
              >
                Get Tickets
              </Button>
            </CardContent>
