  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="flex items-center gap-3 rounded-md border p-3">
        ...
        />
        <div>
          <Label htmlFor="allowPublicDonations" className="cursor-pointer font-medium">Allow Public Donations</Label>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                {campaign.allowPublicDonations && campaign.status === 'active' && (
                  <Badge variant="outline" className="text-xs text-green-700 border-green-300 bg-green-50">Public</Badge>
                )}
              </div>
            </div>
            {canUpdate && (
              <div className="flex gap-1 shrink-0">
                <button onClick={() => setEditCampaign(campaign)} className="p-1 text-muted-foreground hover:text-foreground" title="Edit">
                  <Pencil className="h-3.5 w-3.5" />
                </button>
                {campaign.allowPublicDonations && (
                  <button onClick={() => copyPublicLink(campaign.id)} className="p-1 text-muted-foreground hover:text-foreground" title="Copy public link">
                    {copiedCampaignId === campaign.id ? <Check className="h-3.5 w-3.5 text-green-600" /> : <Copy className="h-3.5 w-3.5" />}
                  </button>
                )}
                {campaign.status === 'active' ? (
                  <button onClick={() => setEndCampaignId(campaign.id)} className="p-1 text-muted-foreground hover:text-destructive" title="End campaign">
                    <StopCircle className="h-3.5 w-3.5" />
                  </button>
                ) : campaign.status === 'completed' && (
                  <button onClick={() => setActivateCampaignId(campaign.id)} className="p-1 text-muted-foreground hover:text-green-600" title="Reactivate">
          ...
          {campaign.allowPublicDonations && !isMember && (
            <div className="rounded-md border bg-muted/40 p-3 space-y-2">
              <p className="text-xs font-medium flex items-center gap-1.5">
                <Share2 className="h-3.5 w-3.5" /> Public donation link
              </p>
              ...
              <div className="flex gap-1.5">
                <button onClick={() => shareWhatsApp(campaign)} className="flex items-center gap-1 text-xs px-2 py-1 rounded border bg-background hover:bg-muted">
                ...
                <button onClick={() => shareFacebook(campaign.id)} className="flex items-center gap-1 text-xs px-2 py-1 rounded border bg-background hover:bg-muted">
