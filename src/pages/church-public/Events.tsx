...
function EventCard({ event, accent }: { event: PublicEvent; accent: string }) {
  ...
  return (
    <article style={{
      ...
      {/* Dark top half */}
      <div style={{
        background: DARK, padding: '20px 18px 16px',
        display: 'flex', alignItems: 'flex-start', gap: 14,
      }}>
        {/* Date badge */}
        <div style={{
          background: accent, borderRadius: 8, padding: '8px 10px',
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <h3 style={{
            ...
            fontSize: 17, lineHeight: 1.25, margin: 0, fontWeight: 700,
      ...
      <div style={{ background: '#fff', padding: '14px 18px 16px', display: 'flex', flexDirection: 'column', flex: 1 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 8, flexWrap: 'wrap' }}>
          <span style={{
            background: `${accent}18`, borderRadius: 5,
            color: DARK, fontSize: 10, fontWeight: 700,
            textTransform: 'uppercase', padding: '3px 9px', letterSpacing: '0.08em',
          }}>
        ...
        <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid #f1f5f9' }}>
          {event.requiresTicket ? (
            <a href={`${FRONTEND}/events/${event.id}`} target="_blank" rel="noopener noreferrer" style={{
              ...
              background: accent, color: DARK, borderRadius: 6,
    ...
    <section id="events" style={{ background: '#f8fafc', padding: '56px 28px 72px' }}>
      <div style={{ maxWidth: 1400, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 14, marginBottom: 28, flexWrap: 'wrap' }}>
          ...
          <a href="#events" style={{
            border: `1px solid ${DARK}22`, borderRadius: 7, color: DARK,
